...
public class UgoRetypeVariableAction extends UgoAbstractDecompilerAction {
    private final UgoDecompilerController controller;
    private final PluginTool tool;

    public UgoRetypeVariableAction(PluginTool tool, UgoDecompilerController controller) {
...
//				return -1;						// Overlaps non-undefined datatype
    ...
    protected boolean isEnabledForDecompilerContext(DecompilerActionContext context) {
        ...
        if (variable == null) {
            Address addr = UgoRenameVariableAction.getStorageAddress(tokenAtCursor, controller);
            if (addr == null) {
                return false;
            }
            variable = UgoRenameVariableAction.forgeHighVariable(addr, controller);
            if (variable == null) {
                return false;
            }
        }
