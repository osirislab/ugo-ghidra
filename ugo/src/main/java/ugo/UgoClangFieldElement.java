...
public class UgoClangFieldElement extends AbstractTextFieldElement {

    private final ClangToken token;

    public UgoClangFieldElement(ClangToken token, AttributedString as, int col) {
        super(as, 0, col);
        this.token = token;
    }
    ...
    public FieldElement substring(int start, int end) {
        ...
        return new ghidra.app.decompiler.component.ClangFieldElement(token, as, column + start);
    }

    @Override
    public FieldElement replaceAll(char[] targets, char replacement) {
        return new ghidra.app.decompiler.component.ClangFieldElement(token, attributedString.replaceAll(targets, replacement),
                column);
    }
}
